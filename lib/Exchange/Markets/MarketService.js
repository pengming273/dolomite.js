...
function (_Service) {
  ...
  _createClass(MarketService, [{
    ...
    value: function onUpdate(callback) {
      this.on('/v1/markets', 'update').build(function (data) {
        return _Market["default"].build(data);
      }).then(callback);
    }
    /*
    ...
    value: function onUpdateAll(callback) {
      ...
      if (!this.allMarketsWS) {
        this.allMarketsWS = new _WSWrapper["default"](function () {
          return _this.getAll();
        });
        this.onUpdate(function (market) {
          return _this.allMarketsWS.trigger();
        });
      }

      this.allMarketsWS.subscribe(callback);
    } // ----------------------------------------------
    // Market Chart
